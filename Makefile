.PHONY: help fits predicts cv values figures test all clean
...
help:
	...
	@echo "  predicts    Generate per-cell / per-condition predictions"
	...
	@echo "  all         fits + predicts + cv + values + figures"
...
predicts:
	$(PY) model/forward/predict_food_forw_intimacy_desire.py
	$(PY) model/inverse/predict_food_inv_intimacy_desire_alt.py
	$(PY) model/inverse/predict_food_inv_desire_intimacy_alt.py
...
all: fits predicts cv values figures
